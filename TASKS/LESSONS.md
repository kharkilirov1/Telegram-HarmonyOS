...
### 2026-07-29 Эмулятор: hdc-мост одноразовый, сирота crash-service валит рестарты
- `hdc kill` в первые ~секунды после boot-complete гостя рушит регистрацию моста НАВСЕГДА для этого запуска: эмулятор подключает hdcd к hdc-серверу один раз (qemu.log: `boot completed` → `input one connect fd` → после kill `recv get close`, ретраев нет). Не перезапускать hdc-сервер, пока эмулятор бутится.
- Осиротевший `emulator-crash-service` (родитель мёртв) держит crash-сокет инстанса — следующие `Emulator -start` молча умирают с exit 0 сразу после «WHP accelerator is operational». Лечение: `taskkill //PID <сирота> //F` перед стартом. Диагностика без вывода — редирект в /dev/null прятал и это; всегда писать лог лаунчера в файл.
- Guest `crash:system freeze` (Emulator.log CrashClient) при ~5ГБ свободного RAM хоста: hdcd не отвечает, таргет Offline, за 3 мин сам не оживает — сразу `-stop`+start. Витнесс-паттерн «кадр разминулся с транзишном»: скрин через ~1с после тапа может показать ещё чат — переснять через 2-3с прежде, чем судить об отсутствии галереи.

### 2026-07-29 Урок 84 в V2-обличье: замена массива убивает live-refresh переиспользованных узлов
- Открытая галерея «не видела» файловые обновления: ForEach key (index+messageId) стабилен по дизайну, а рефреш заменял `@Param mediaItems` новым массивом — узлы переиспользовались со СТАРЫМИ замыканиями item-объектов. В V1-чате это решается key-bump'ом; в V2 правильное решение противоположное — сохранить identity объектов: `@ObservedV2/@Trace` на волатильных полях + мутация существующих объектов in place (`mergeGalleryItemsInPlace`), замена массива только при смене состава.
- Симптом-пара для диагностики: «строка чата обновляется live, галерея с тем же файлом — нет» = дельта доезжает до стора, гибнет на identity узла. Ищи, кто заменяет массив/объект при стабильном key.
- Runtime-охота вслепую по «кликнул-снял» ловит ложные интерпретации: «чёткое фото в галерее» оказалось скачанным thumbnail (preview kind='thumbnail', без blur), а не live-подменой. Прежде чем записать «работает», проверь, какая ветка рендера это нарисовала.
...
### 2026-07-29 getIntPluralStringValueSync: число надо передавать дважды
- `getIntPluralStringValueSync(resId, count)` выбирает CLDR-форму по count, но НЕ подставляет его в `%d` — рантайм отрисовал «%d участник» буквально. Подстановка требует count вторым (форматным) аргументом: `(resId, count, count)`. Витнессить plural-строки только рантаймом — компиляция и RAG-примеры этот нюанс не показывают.
- После нескольких force-stop→start подряд TDLib-пайплайн может встать целиком (все команды таймаутят 30000ms, включая closeChat) — это состояние стенда, а не последнего фикса; не диагностировать свой код по такому прогону, перезапускать гостя целиком.
