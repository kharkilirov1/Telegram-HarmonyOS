...
- [ ] Проверить, что playback-обновления (voice/audio isPlaying через notifyItemChangedByMessageId) живы после key-штампа — их канал тот же notifyDataChange со стабильным key (подозрение по уроку 84; если сломаны — перевести на key-штамп или VO-поле)
- [ ] Видеоплеер полноэкран/галерея по тапу до скачивания — прогнать следующим заходом
