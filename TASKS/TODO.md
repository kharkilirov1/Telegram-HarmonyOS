...
- [ ] Shared Media полиш: докачка тумб верхних (свежих) ячеек медленная/застревает — проверить выбор размера (sizes['m'] тяжёлый? голова очереди блокирует) + лог ошибок downloadThumb; вкладки Files/Links/Voice (фильтры Document/Url/VoiceNote — командный слой уже готов); тап по тайлу → открытие полноэкрана
