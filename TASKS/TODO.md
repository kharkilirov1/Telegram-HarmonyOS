...
- [ ] Композер-полиш дальше: категории эмодзи (лица/жесты/животные — нужен полный каталог), кнопка камеры рядом с микрофоном (iOS), анимация появления панелей
...
- [ ] Подсветка jump-цели на стикер-строках не видна (α=0.15 на чёрном фоне) — усилить flash для не-бабловых строк или рисовать рамку
...
- [x] **Shared Media / Media — РЕАЛИЗОВАН (`f1d1d7b`, тик 39)**: filterType в SearchChatMessagesPayload (сериализуется в SearchMessagesFilter), searchChatMedia+downloadThumb в usecase, TgSharedMediaPage (грид 3 колонки, video-бейджи длительности, последовательная докачка тумб, свой safe-area header, route_map). Witness: Москвач → профиль → Media → сетка с реальными тумбами (забор 0:23, ежи 0:20, циклон 0:05, сова 0:10), back работает
...
- [ ] Shared Media полиш: докачка тумб Media-грида подтормаживает на свежих ячейках (размер sizes['m']? голова очереди) + лог ошибок downloadThumb; тап по тайлу/строке → открытие контента
