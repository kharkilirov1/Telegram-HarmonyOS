...
- [ ] **[Инцидент 2026-07-02] Эмулятор разлогинен**: `hdc install -r` тестового HAP пересоздал бандл и стёр данные приложения (TDLib-сессию). Нужен ручной вход пользователя (номер+код или QR). Урок №60. Попутно: fresh-install логин-экран отрендерился корректно (частичный witness первого пункта MVP-чеклиста)
