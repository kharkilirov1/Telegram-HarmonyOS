...
- [ ] **[P3] Сверить open-position семантику** (уточнено 2026-07-03): divider «Не прочитано» стабильно работает в каналах (HarmonyOSHub, Москва сегодня — открытие ровно на нём); открытый вопрос только по группам (Huawei HarmonyOS открылась на «22 июн» без разделителя) — проверить, был ли у группы unreadCount на момент открытия, прежде чем считать дефектом
