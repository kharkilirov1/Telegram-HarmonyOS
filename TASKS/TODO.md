...
- [ ] **[P3] Сверить open-position семантику** (уточнено 2026-07-02): unread-разделитель «Не прочитано» СУЩЕСТВУЕТ и виден в канале HarmonyOSHub; в группе Huawei HarmonyOS чат открылся на верхе окна («22 июн») без разделителя — проверить выбор якоря (unread vs top-of-window) и показ разделителя в группах
...
- [ ] **[P4-b] Пути загруженных тумб не доезжают до открытого таймлайна**: 432× `FileDownloadedEvent emitted`, но бабблы фото/альбома остаются плейсхолдерами, и рескан-цепочка вотчера молчит после загрузок (признак: `state.messages` не меняется на `fileDownloaded`). `contentMatchesFileId`/`applyFilePathToContent` для одиночных фото выглядят корректно — нужен трассирующий hilog в pre-scan filesReducer (hit/miss по fileId) и проверка, что fileId из `updateFile` совпадает с `photoThumbFileId` контента. Альбом = отдельные photo-сообщения, поэтому ветка общая
...
- [ ] **R1.5 (новый гейт, см. план): дефект-лист внешки** — верхние табы, композер, чат-лист против iOS-референса; согласовать с пользователем до правок
