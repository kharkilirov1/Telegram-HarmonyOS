...
- [ ] **[ДЕФЕКТ] TextArea держит собственный буфер**: программный setDraftText рисует текст, но первый ввод начинает С ПУСТОГО буфера (prop не синхронизирует внутреннее состояние). План: двусторонний биндинг text (`!!`/$$ API 18) или TextAreaController-подход; после фикса — доснять live-witness edit («edited»→«edited2» на глазах)
